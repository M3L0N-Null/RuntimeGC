...
namespace RuntimeGC
{
    internal static class CleanserUtil
    {
        ...
        private static FieldInfo activeEntries = typeof(BattleLog).GetField("cachedActiveEntries", BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        public static int RemoveAllBattleLogEntries()
        {
            ...
            //{
            //    Log.Message("Entries Count: " + b.Entries.Count.ToString());
            //    a += b.Entries.CountAllowNull();
            //    Log.Message(b.GetType().ToString());
            //    Log.Message(a.GetType().ToString()); ;
            //    battles.SetValue(Find.BattleLog, new List<Battle>());
            //    Log.Message("Passed Setting Value of BattleLog");
            //    activeEntries.SetValue(Find.BattleLog, null);
            //    Log.Message(activeEntries.GetValue(Find.BattleLog).ToString());
            //}
            //Log.Message("Return a string" + a.ToString());
            //return a;
            //}
            //catch
            //{
            //    Log.Message("We're in the try in the Remove Battle Log Entries catch uh oh");
            //    return 666;
            //}
            ...
            foreach(Battle battle in Find.BattleLog.Battles)
            {
                ...
                else
                {
                    ...
                    activeEntries.SetValue(Find.BattleLog, null); // exception on this 
                }
            }
            return a;
            }
            ...
            {
                Log.Message("RuntimeGC Error Start: " + Environment.NewLine + ex.StackTrace + Environment.NewLine + ex.Message + Environment.NewLine + ex.Source + Environment.NewLine + ex.Data + Environment.NewLine + Environment.NewLine + "RuntimeGC End.");
                return 0;                
            }
        }
